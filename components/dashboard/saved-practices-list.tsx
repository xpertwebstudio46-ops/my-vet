...
import { Bookmark, PawPrint, Search, Trash2 } from 'lucide-react'
...
export function SavedPracticesList({ mode }: { mode: 'favourites' | 'saved' }) {
  ...
  const Icon = mode === 'favourites' ? PawPrint : Bookmark
  ...
  return <div className="min-w-0">
    ...
    {loading ? <Card className="p-8 text-center text-sm text-muted-foreground">Loading saved practices...</Card> : visible.length ? <div className="grid min-w-0 gap-4 sm:grid-cols-2 xl:grid-cols-3">{visible.map((practice) => <div key={practice.id} className="relative min-w-0"><PracticeCard practice={toCard(practice)} favourited showSaveAction={false} /><button type="button" onClick={() => void remove(practice)} className="absolute bottom-4 left-4 z-10 inline-flex h-9 items-center gap-1.5 rounded-full border bg-white px-3 text-xs font-semibold text-red-600 shadow-sm"><Trash2 className="size-3.5" />Remove</button></div>)}</div> : <EmptyState icon={Icon} title={query ? 'No matching practices' : 'No saved practices yet'} description={query ? 'Try another search term.' : 'Save an approved practice from the dashboard or directory and it will appear here.'} action={<Link href="/find-a-vet" className="rounded-lg bg-[#064071] px-4 py-2 text-sm font-semibold text-white">Browse practices</Link>} />}
  </div>
}
