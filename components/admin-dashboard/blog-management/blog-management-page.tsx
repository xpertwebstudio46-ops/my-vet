...
export function BlogManagementPage() {
  ...
  async function savePost(input: BlogPostInput) {
    ...
    try {
      const post = await apiClient<BlogPost>(editing ? `/api/admin/blog/${editing.id}` : '/api/admin/blog', { method: editing ? 'PUT' : 'POST', body: JSON.stringify({ title: input.title, excerpt: input.excerpt, content: input.content, category: input.category, status: input.status, ...(asset ? { coverAssetId: asset.id } : {}) }) })
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="overflow-hidden p-0"><div className="overflow-x-auto"><table className="w-full min-w-[840px] text-left"><thead className="border-b text-xs font-semibold uppercase text-muted-foreground"><tr><th className="px-5 py-4">Cover</th><th className="px-5 py-4">Blog</th><th className="px-5 py-4">Category</th><th className="px-5 py-4">Status</th><th className="px-5 py-4">Published</th><th className="px-5 py-4 text-right">Actions</th></tr></thead><tbody>{posts.map((post) => <tr key={post.id} className="border-b border-gray-200/80"><td className="px-5 py-4"><span className="relative block h-12 w-20 overflow-hidden rounded-md bg-slate-100"><Image src={post.coverUrl || '/placeholder.svg'} alt="" fill sizes="80px" className="object-cover" /></span></td><td className="px-5 py-4"><p className="text-sm font-semibold text-black">{post.title}</p><p className="mt-1 max-w-xl text-xs text-muted-foreground">{post.excerpt}</p></td><td className="px-5 py-4"><span className="rounded-full bg-teal-50 px-2.5 py-1 text-xs font-semibold text-[#087b75]">{blogCategoryLabel(post.category)}</span></td><td className="px-5 py-4"><StatusPill status={post.status} /></td><td className="px-5 py-4 text-sm text-muted-foreground">{post.publishedAt ? new Date(post.publishedAt).toLocaleDateString('en-GB') : 'Not published'}</td><td className="px-5 py-4"><div className="flex justify-end gap-2"><AdminIconButton label="Edit blog" onClick={() => setEditing(post)}><Pencil className="size-4" /></AdminIconButton><AdminIconButton label="Delete blog" onClick={() => setDeleting(post)}><Trash2 className="size-4" /></AdminIconButton></div></td></tr>)}</tbody></table></div></Card>
