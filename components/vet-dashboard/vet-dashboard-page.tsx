...
import { CalendarCheck, Eye, MapPin, MessageSquare, PhoneCall, Star } from 'lucide-react'
import { Card } from '@/components/dashboard/ui'
import { apiClient, ApiClientError } from '@/lib/api/client'
import type { Paginated } from '@/lib/api/types'

type PracticeStatus = 'PENDING' | 'APPROVED' | 'REJECTED' | 'SUSPENDED' | 'ARCHIVED'
type Dashboard = { practice: { id: string; name: string; rating: string; reviewCount: number; status: PracticeStatus; moderationReason: string | null; emergencyNumber: string | null; emergencyCalloutAddress: string | null }; stats: { upcomingAppointments: number; pendingReviews: number; views: number; contacts: number } }
...
export function VetDashboardPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {data && <Card className="grid gap-4 p-5 md:grid-cols-[1fr_1fr_auto] md:items-center"><div className="min-w-0"><p className="flex items-center gap-2 text-sm text-muted-foreground"><PhoneCall className="size-4 text-[#01AEAD]" />Emergency number</p><p className="mt-1 break-words font-semibold text-black">{data.practice.emergencyNumber || 'Not set'}</p></div><div className="min-w-0"><p className="flex items-center gap-2 text-sm text-muted-foreground"><MapPin className="size-4 text-[#01AEAD]" />Emergency callout address</p><p className="mt-1 break-words font-semibold text-black">{data.practice.emergencyCalloutAddress || 'Not set'}</p></div><Link href="/vet-dashboard/opening-hours" className="rounded-md border px-4 py-2 text-center text-sm font-semibold text-[#064071]">Edit emergency</Link></Card>}
